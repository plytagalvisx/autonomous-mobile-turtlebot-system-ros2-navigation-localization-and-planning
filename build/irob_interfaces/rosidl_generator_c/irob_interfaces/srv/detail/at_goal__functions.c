  ...
  irob_interfaces__srv__AtGoal_Request__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    irob_interfaces__srv__AtGoal_Request * data =
      (irob_interfaces__srv__AtGoal_Request *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!irob_interfaces__srv__AtGoal_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          irob_interfaces__srv__AtGoal_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
  ...
  irob_interfaces__srv__AtGoal_Response__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    irob_interfaces__srv__AtGoal_Response * data =
      (irob_interfaces__srv__AtGoal_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    ...
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!irob_interfaces__srv__AtGoal_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          irob_interfaces__srv__AtGoal_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
